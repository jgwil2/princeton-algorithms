...
public class Point implements Comparable<Point> {
    ...
    public double slopeTo(Point that) {
        ...
        else if (this.x == that.x) return Double.POSITIVE_INFINITY; // avoid division by zero
        else return (float) (this.y - that.y) / (this.x - that.x);
    }
    ...
    private static class SlopeOrder implements Comparator<Point> {
        ...
        public int compare(Point q, Point r) {
            return Double.compare(p.slopeTo(q), p.slopeTo(r));
        }
    }
    ...
    public static void main(String[] args) {
        ...
        assert origin.slopeTo(p) == 1;
        assert origin.slopeTo(q) == 0.5;
        assert p.compareTo(p) == 0;
        assert p.compareTo(q) == -1; // p is less than q
        Comparator<Point> c = origin.slopeOrder();
        assert c.compare(p, p) == 0;
        assert c.compare(p, q) == 1; // 1 is greater than 1/2
    }
}